...
from ..geometry.geom3d import dist_to_volume
import skimage
...
class StarDistBase(BaseModel):
    ...
    def predict(self, img, axes=None, normalizer=None, n=None, show_progress=True, **predict_kwargs):
        """Predict.
        ...
        show_progress: bool or callable
        ...
        x, axes, axes_net, axes_net_div_by, _permute_axes, resizer, n, grid, grid_dict, channel, predict_direct, tiling_setup = \
            self._predict_setup(img, axes, normalizer, n, show_progress, predict_kwargs)
    ...
    def predict_sparse(self, img, volume_min = None, volume_max = None, prob_thresh=None, axes=None, normalizer=None, n=None, results=None, show_progress=True, b=2, **predict_kwargs):
        ...
        if self.config.n_dim == 2:
            ...
            volume_temp = dist_to_volume(dist, rays)
            if volume_min is not None :
                inds2   = _ind_prob_thresh(volume_temp, volume_min, b=b)
                inds = inds & inds2
            if volume_max is not None:
                inds3   = _ind_prob_thresh(-volume_temp, -volume_max, b=b)
                inds = inds & inds3
            volume_temp = volume_temp.flatten()
            proba_array = prob*inds
        
            mask_local_max = skimage.morphology.local_maxima(proba_array, indices = False)
            mask_reduce_pixels = np.zeros((mask_local_max.shape[0], mask_local_max.shape[1], mask_local_max.shape[2]), dtype=bool) #true array
            mask_reduce_pixels[:,::2,::2] = True
            final_mask_pixel_reducing = (mask_reduce_pixels & inds) | mask_local_max 
            proba = prob[final_mask_pixel_reducing].copy() #proba = prob[inds].copy()
        
            dista = dist[final_mask_pixel_reducing].copy()  #dist[inds].copy()
            
            _points = np.stack(np.where(final_mask_pixel_reducing), axis=1)  #where(inds) 
            ##here________________________________________________________________________________________
            pointsa = (_points * np.array(self.config.grid).reshape((1,len(self.config.grid))))
            
            volume = np.array(volume_temp)
            volume = np.delete(volume, np.where(volume == 0)) #[value for value in volume_temp if value != 0] 
        ...
        if self._is_multiclass():
            ...
            prob_classa = None
            return proba, dista, pointsa, volume, results


    def predict_instances(self, img, axes=None, normalizer=None,
                          sparse=True, volume_min = None, volume_max = None,
                          ...
                          overlap_label=None, return_predict=False, results=None):
        ...
        """
        try:
            threshs = json.load(open(str(self.logdir / 'thresholds.json'), 'r'))
            threshs['prob'] = prob_thresh
            threshs['nms'] = nms_thresh
            with open(str(self.logdir / 'thresholds.json'), "w") as outfile:
                json.dump(threshs, outfile)
        except:
            pass
        self.thresholds = dict (
            prob = prob_thresh,
        ...
        print("Using values: prob_thresh={prob:g}, nms_thresh={nms:g}.".format(prob=self.thresholds.prob, nms=self.thresholds.nms))
        ...
        _axes         = self._normalize_axes(img, axes) if self.config.n_dim > 2 else self._normalize_axes(img[0], axes)
        _axes_net     = self.config.axes
        _permute_axes = self._make_permute_axes(_axes, _axes_net)
        if self.config.n_dim == 2:
            _shape_inst   = tuple(s for s,a in zip(_permute_axes(img[0]).shape, _axes_net) if a != 'C')
        else:
        ...
        start_net = time.time()
        if sparse:
            res = self.predict_sparse(img, axes=axes, normalizer=normalizer, volume_min=volume_min, volume_max=volume_max, 
                                      prob_thresh=prob_thresh, results=results, show_progress=show_progress, **predict_kwargs)
        else:
            res = self.predict(img, axes=axes, normalizer=normalizer,
                               show_progress=show_progress, **predict_kwargs)
            res = tuple(res) + (None,)
        tt_time = time.time() - start_net
        print(f'Prections done ! Elapsed time {tt_time} seconds')

        if self._is_multiclass():
            prob, dist, prob_class, points = res
        else:
            prob, dist, points, volume, results = res
            prob_class = None

        start_qhull = time.time()
        res_instances = self._instances_from_prediction(_shape_inst, volume, prob, dist, results,
                                                        points=points,
                                                        ...
                                                        **nms_kwargs)
        end_qhull = time.time()
        print('Time for qhull ' + str(end_qhull - start_qhull))
        if return_predict:
